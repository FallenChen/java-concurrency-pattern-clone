package com.garry.features.futures;
 ...
 * Future interface with the CompletionState interface, supporting dependent actions that
 * trigger upon its completion, similarly to a callback.
 *
 * Important: Specify an Executor for async methods when available.All async methods
 * without an explicit Executor argument are performed using the ForkJoinPool.commonPool();
 *
 * Important 2:Mostly of the CompletableFuture methods returns a new CompletableFuture
 *
 ...
 *
 * xxxAsync(..., Executor executor);// Executed in the specified Executor,good for Java EE
 *
 * = supply x run =
 * supplyAsync(Supplier<U> supplier); // will complete asynchronously by calling supplier
 *
 ...
 *
 * thenApply: accepts a consumer to the result value;
 *
 ...
 *
 * = Mapping value from Two Futures =
 *
...
public class UsingCompletableFuture {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        CompletableFuture
                .supplyAsync(getVal)
                .acceptEitherAsync(CompletableFuture.supplyAsync(getVal2, executor),
                        (firstToBeReady) -> System.out.println(firstToBeReady), executor);
        executor.shutdown();
